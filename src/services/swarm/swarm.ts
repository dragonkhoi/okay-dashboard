...
import { AGENT_PREFIX, PLANNER_AGENT, SERVER_TOOL_NAME_SEPARATOR } from "../../constants";
...
export class Swarm {
    ...
    async run({ agentName, messages, contextVars = {}, modelOverride, maxTurns = Infinity, stream = false, debug = false }: { agentName: string, messages: any[], contextVars?: Record<string, any>, modelOverride?: string, maxTurns?: number, stream?: boolean, debug?: boolean }) {
      ...
      while (numTurns < maxTurns && currentAgent) {
        ...
        console.log("CONVERSATION HISTORY", JSON.stringify(reducedConversationHistory, null, 2));
        ...
        for (let i = 0; i < response.content.length; i++) {
            ...
            if (contentBlock.type === "text") {
                const message = {...contentBlock, role: "assistant", content: contentBlock.text, sender: currentAgent.name};
                if (message.content) {
                ...
                const message = {...toolUseBlock, role: "assistant", content: 
                    [{type: "tool_use", name: toolUseBlock.name, input: toolUseBlock.input, id: toolUseBlock.id}],
                    sender: currentAgent.name};
                conversationHistory.push(message);
                if (debug) console.log(`${currentAgent.name}:`, message);
                // call tool 
                const toolMessage = await this.handleToolCall(toolUseBlock, currentAgent);
                
                conversationHistory.push(toolMessage);
                if (toolMessage.content[0].content && typeof toolMessage.content[0].content === "string" && toolMessage.content[0].content.startsWith(AGENT_PREFIX)) {
                    const agentName = toolMessage.content[0].content.split(SERVER_TOOL_NAME_SEPARATOR)[1];
      ...
      return { agentName: currentAgent.name || PLANNER_AGENT, messages: conversationHistory };
    }

    async getChatCompletion({agent, conversationHistory, contextVariables, modelOverride, stream, debug}: {agent: Agent, conversationHistory: any[], contextVariables: Record<string, any>, modelOverride?: string, stream?: boolean, debug?: boolean}) {
        ...
        const chatCompletionParams = {systemPrompt, model: modelOverride || agent.model, messages: conversationHistory, tools: functions, toolChoice: agent.toolChoice, stream}
        const response = await this.createChatCompletion(chatCompletionParams)
        return response;
    }

    async createChatCompletion({systemPrompt, model, messages, tools, toolChoice = {type: "auto"}, stream = false }: {systemPrompt: string, model: string, messages: any[], tools: any[], toolChoice?: ToolChoice, stream?: boolean}) {
    ...
    async handleToolCall(toolUseBlock: ToolUseBlock, currentAgent: Agent) {
        ...
        if (toolCall.serverName === AGENT_PREFIX) {
            return {
                type: "tool_result",
                role: "user",
                content: [{type: "tool_result", content: toolUseBlock.name + SERVER_TOOL_NAME_SEPARATOR + "COMPLETED_HANDOFF", tool_use_id: toolUseBlock.id}],
                sender: currentAgent.name,
            }
        } else {
            ...
            const toolMessage = {
                type: "tool_result",
                role: "user",
                content: [{type: "tool_result", content: typeof toolResult === "string" ? toolResult : JSON.stringify(toolResult), tool_use_id: toolUseBlock.id}],
                sender: currentAgent.name,
            }
            return toolMessage;
        }
