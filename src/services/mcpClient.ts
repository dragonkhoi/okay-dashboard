...
export class McpClientService {
  ...
  constructor(serverConfig: McpServerConfig) {
    this.serverConfig = {...serverConfig, name: cleanServerName(serverConfig.name)};
  }
   ...
   */
  async callTool(params: { serverName: string; toolName: string; arguments: Record<string, any> }, retryCount = 0): Promise<any> {
    const MAX_RETRIES = 3;
    const RETRY_DELAY = 1000;

    try {
        console.log(`Using tool: ${params.toolName} from server: ${params.serverName} with args: ${JSON.stringify(params.arguments)}`);

        const response = await this.client.callTool({name: params.toolName, arguments: params.arguments});
        ...
        console.error(`Failed to call tool ${params.toolName} from server ${params.serverName}:`, error);
